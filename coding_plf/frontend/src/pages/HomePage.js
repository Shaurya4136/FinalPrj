    ...
    if (!isHovered) {
      scrollInterval = setInterval(() => {
        if (eventsRef.current) {
          ...
          // Reset scroll position if scrolled to the end
          if (eventsRef.current.scrollLeft + eventsRef.current.clientWidth >= eventsRef.current.scrollWidth) {
            eventsRef.current.scrollLeft = 0;
          }
        }
      }, 30); // Adjust the interval for desired scrolling speed
    }
